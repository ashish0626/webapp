    ...
    stage('Build Docker Containerize and push to harbor') {
      steps {
        container('kaniko') {
          ...
          sh "/kaniko/executor --dockerfile Dockerfile --context `pwd` --skip-tls-verify --destination=${env.HARBOR_URL}/library/samples/web-core:v1.0.${env.BUILD_ID}"
        }
      }
    }
      ...
      steps {
        container('maven') {
          sh """
            ...
            cd deploy
            # update values.yaml
            sed -i -r 's,repository: (.+),repository: ${env.HARBOR_URL}/library/samples/web-core,' values.yaml
